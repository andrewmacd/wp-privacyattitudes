...
Presentation website: https://bit.ly/cambridgeprivacy
...
table.text[[7]] <- c("Q7", "Are you a member or probationary member of the CCP?")
table.text[[8]] <- c("Q8", "Here is a table showing the range of monthly incomes that people have. Which of the letters on this table best represents the total monthly income of your household (after tax)?")
table.text[[9]] <- c("Q9", "What is your residency status?")
...
datasummary_balance(~ Year, data=gp.qs, dinm_statistic = "p.value",
    ...
    tab_style(style = cell_fill(color = "pink"),
              locations = cells_body(rows = p <= 0.05, columns = 6))
```
...
table.text[[4]] <- c("Q4", "I don’t notice government use of technology to monitor my behavior in my daily life.")
...
Private vs. public monitoring questions

```{r ppqs}
table.text <- list()

table.text[[1]] <- c("Q1", "How comfortable are you with the central government knowing personal details about your activity online?")
table.text[[2]] <- c("Q2", "How comfortable are you with the local government knowing personal details about your activity online?")
table.text[[3]] <- c("Q3", "How comfortable are you with private companies knowing personal details about your activity online?")
...
table.text[[5]] <- c("Q5", "It doesn’t bother me if private companies sell my user data to third parties if it will allow me to use their applications for free.")
...
Private vs. public monitoring data
...
misc.qs <- privacy %>% 
  ...
  select(`Central government tracking - comfort` = track4,
...
kable(listexp.text, col.names = NULL, escape=FALSE) %>% 
  kable_styling() %>% 
  column_spec(1, bold = T, border_right = T)
```
...
```{r regtrustacceptvars}
table.text <- list()

table.text[[1]] <- c("DV1", "Overall, I’m happy with the performance of the central government")
table.text[[2]] <- c("DV2", "Overall, I’m happy with the performance of my local government")
table.text[[3]] <- c("DV3", "There are good reasons for the central government to monitor the activity of users online")
table.text[[4]] <- c("DV4", "There are good reasons for the local government to monitor the activity of users online")
table.text[[5]] <- c("DV5", "There are good reasons for private companies to monitor the activity of users online")
...
```{r trustmenu, out.width = "100%"}
selectInput(
  'model1', label = 'Variable: ',
  choices = c("All", "Age", "Demographics", "Location", "Year"), 
  selected = "All"
)
...
output$trustmenu <- renderUI({
  
  if(input$model1=="All") {
    modelsummary(mod_list_soc_update, coef_rename = c("age" = "Age",
                                     "educationMiddle school" = "EDU: Middle school",
                                     "educationHigh school" = "EDU: High school",
                                     "educationUniversity" = "EDU: University",
                                     "educationAdvanced studies/Graduate school" = "EDU: Grad school",
  ...
  } else if(input$model1=="Demographics"){
        modelsummary(mod_list_soc_update, coef_rename = c("age" = "Age",
                                     "educationMiddle school" = "EDU: Middle school",
                                     "educationHigh school" = "EDU: High school",
                                     "educationUniversity" = "EDU: University",
...
table.text[[1]] <- c("DV1", "How closely do you think the central government tracks your online activity?")
table.text[[2]] <- c("DV2", "How closely do you think the local government tracks your online activity?")
table.text[[3]] <- c("DV3", "How closely do you think private companies track your online activity?")
table.text[[4]] <- c("DV4", "How comfortable are you with the central government knowing personal details about your activity online?")
table.text[[5]] <- c("DV5", "How comfortable are you with the local government knowing personal details about your activity online?")
table.text[[6]] <- c("DV6", "How comfortable are you with private companies knowing personal details about your activity online?")
...
```{r trackingmenu, out.width = "100%"}
selectInput(
  'model2', label = 'Variable: ',
  choices = c("All", "Age", "Demographics", "Location", "Year"), 
  selected = "All"
)

uiOutput("trackingmenu")
```

```{r trackingmenudef, context="server"}
...
mod6 <- lm(data=reg.tracking.data, as.numeric(track1) ~ age + education + income + sex  
           + party.member.status + location + year)
mod7 <- lm(data=reg.tracking.data, as.numeric(track2) ~ age + education + income + sex 
           + party.member.status + location + year)
mod8 <- lm(data=reg.tracking.data, as.numeric(track3) ~ age + education + income + sex  
           + party.member.status + location + year)
mod9 <- lm(data=reg.tracking.data, as.numeric(track4) ~ age + education + income + sex  
           + party.member.status + location + year)
mod10 <- lm(data=reg.tracking.data, as.numeric(track5) ~ age + education + income + sex  
           + party.member.status + location + year)
mod11 <- lm(data=reg.tracking.data, as.numeric(track6) ~ age + education + income + sex  
           + party.member.status + location + year)
...
mod_list_soc_tracking <- modelsummary(mod.tracking, output = "modelsummary_list")
mod_list_soc_update_tracking <- as.list(mod_list_soc_tracking)
mod_list_soc_update_tracking <- replace_estimates(mod_list_soc_update_tracking)

output$trackingmenu <- renderUI({
  
  if(input$model2=="All") {
    modelsummary(mod_list_soc_update_tracking, coef_rename = c("age" = "Age",
                                     "educationMiddle school" = "EDU: Middle school",
                                     "educationHigh school" = "EDU: High school",
                                     "educationUniversity" = "EDU: University",
                                     "educationAdvanced studies/Graduate school" = "EDU: Grad school",
  ...
  } else if(input$model2=="Demographics"){
        modelsummary(mod_list_soc_update_tracking, coef_rename = c("age" = "Age",
                                     "educationMiddle school" = "EDU: Middle school",
                                     "educationHigh school" = "EDU: High school",
                                     "educationUniversity" = "EDU: University",
...
- $\mathbf{H1_a}: government\:monitoring \rightarrow high\:citizen\:privacy\:concern$
  ...
  + **Evidence**: High trust in government even after zero Covid<br><br> 
- $\mathbf{H1_b}: any\:type\:of\:monitoring \rightarrow low\:citizen\:privacy\:concern$
  + **Reason**: Historical lack of privacy; culture
  + **Result**: ![](www/no.png){width=20px}
  + **Evidence**: Low trust in private corporation use of data<br><br> 
- $\mathbf{H1_c}: government\:monitoring \rightarrow low\:citizen\:privacy\:concern$
  + **Reason**: High level of trust in government
  + **Result**: ![](www/yes.png){width=20px}
  + **Evidence**: Increasing acceptance of monitoring after zero Covid<br><br> 
- $\mathbf{H2}: 2021\:to\:2023 \rightarrow trust\:in\:government\downarrow$
  + **Reason**: Covid effects
  + **Result**: ![](www/no.png){width=20px} / ![](www/yes.png){width=20px}
  + **Evidence**: Modest decrease in trust, cause unclear<br><br>
- $\mathbf{H2_b}: 2021\:to\:2023 \rightarrow acceptance\:of\:goverment\:monitoring\downarrow$
  + **Reason**: Covid effects
  + **Result**: ![](www/no.png){width=20px}
  + **Evidence**: Acceptance of government monitoring increased<br><br> 
- $\mathbf{H3}: Living\:in\:city\:hit\:hard\:by\:Covid \rightarrow trust\:and\:monitoring\:acceptance\downarrow$ 
  + **Reason**: Heterogeneous effects?
  + **Result**: ![](www/yes.png){width=20px}
  + **Evidence**: Shanghai residents have strongly divergent opinions
